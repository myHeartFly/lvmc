...
class Grablailaihui extends CI_Controller {
	private $source = '来来会';
	private $codePrefix = 'LL';
	private $start;
	private $end;
	...
	public function index()
	{
		...
		foreach($startCities as $startCity => $startParams) {	
			foreach($endCities as $endCity => $endParams) {
				...
				$entryURL = "http://www.lailaihui.com/Search?startDateBegin=&startDateEnd=&priceBegin=&priceEnd=&GoCity={$startParams['map']}&target={$endParams['map']}&price=all&month=all&type=6&SortBy=1&orderColum=&KeyWord=&StartEndDate=&ntype=2";
				$dom = $this->_getDOM($entryURL);
				
				while (true) {
					$nolist = $dom->find('.hotLine .noresut');
					if(!empty($nolist)) {
						break;
					}
					
					$list = $dom->find('div.hotList ul li');
					foreach ($list as $li) {
						$a = $li->find('h3 a', 0);
						$this->_grabdetail('http://www.lailaihui.com'. $a->href, $entryURL); // 抓取单页数据
					}
					$nextPage = $dom->find('div.quotes a[id=NextPage]', 0);
					if ($nextPage) {
						$dom = $this->_getDOM($entryURL + '&pageNumber=' + $nextPage->rel);
					}
					else {
						break;
					}
		...
		echo 'update lailaihui success'.date("Y-m-d H:i:s")."\n";
	}
